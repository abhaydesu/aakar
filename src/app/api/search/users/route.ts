...
export async function POST(req: Request) {
  try {
    ...
    const techs = Array.isArray(body.techs) ? body.techs.map((t: string) => t.toLowerCase()) : [];
    ...
    // Find all users (excluding the recruiter themselves)
    ...
    // For each user, count matching techs from their credentials
    for (const user of users) {
      ...
      for (const cred of creds) {
        ...
        for (const s of skills) userTechs.add(s.toLowerCase());
      }
