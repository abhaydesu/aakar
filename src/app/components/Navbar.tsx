...
export const Navbar = () => {
  ...
  useEffect(() => {
    const fetchUnread = async () => {
      ...
      try {
        const res = await fetch('/api/chat/unread-chats', { method: 'GET', credentials: 'include' });
        if (!res.ok) {
          setUnreadCount(0);
          return;
        }
  ...
  const navItems = [{ title: 'Contact', href: '/contact' }, { title: 'About us', href: '/about' }];

  const buttonBase = "text-lg py-2 px-4 rounded-full border-2 transition-all duration-300 flex items-center gap-2";
  const buttonLoggedIn = `${buttonBase} border-neutral-200 text-neutral-200 hover:bg-neutral-200 hover:text-green-900`;
  const buttonNav = `${buttonBase} border-transparent hover:border-neutral-200 text-lg`;
        ...
        <div className="flex flex-row gap-4 sm:gap-8 items-center">
          {!session?.user ? (
            <>
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`${buttonNav} ${isActive ? "bg-neutral-200 text-green-900 border-neutral-200" : "border-transparent"}`}
                  >
                    {item.title}
                  </Link>
                );
              ...
              <Link
                href="/signin"
                className={`${buttonNav} bg-neutral-200 text-green-900 border-neutral-200 hover:bg-neutral-300`}
              >
                Sign In
              </Link>
            </>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-semibold leading-none text-white bg-amber-600 rounded-full shadow">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-40 bg-white rounded-lg shadow-lg text-neutral-800 overflow-hidden">
                    <Link
                      href="/about"
